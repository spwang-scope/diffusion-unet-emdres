...
class UNetConditional(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, base_filters=64, dropout=0.2, num_class = 10, class_emd_size = 64):
        super(UNetConditional, self).__init__()

        self.class_emd = nn.Embedding(num_class, class_emd_size)
        self.emd_to_2d = nn.Linear(class_emd_size,base_filters**2)
    ...
    def forward(self, x, class_label):
        batch_size, ch, w, h = x.shape

        class_cond = self.class_emd(class_label)
        class_cond = self.emd_to_2d(class_cond)

        # add class embedding to input
        #print(class_cond.shape)
        # add batch dimension
        class_cond = class_cond.unsqueeze(1)
        #print(class_cond.shape)
        class_cond = class_cond.view(batch_size,ch,w,h)
        x = x + class_cond
        #class_cond = class_cond.view(batch_size,class_cond.shape[1],1,1).expand(batch_size,class_cond.shape[1],w,h)
        #x = torch.cat((x, class_cond), 1) # cat along first dimension (channel+1)
